...
class RTObject:
    ...
    def GetObjTypeid(self,object_id):
        '''Translate Object ID to Object Name'''
        #Get interface id
    ...
    def GetServiceTag(self,object_id):
        '''Translate Object ID to Object Name'''
        #Get interface id
        sql = "SELECT asset_no FROM Object WHERE id = %d" % (object_id)
        result = self.db_query_one(sql)
        if result != None:
            object_asset_no = result[0]
        else:
            object_asset_no = None

        return object_asset_no
    ...
    def GetObjectTags(self,object_id):
        ...
        return result
    def GetLocationTagid(self,tag):
        #Get tag id
        sql = "SELECT id FROM TagTree  WHERE parent_id=3 and tag='%s'" % (tag)
        result = self.db_query_one(sql)
        if result != None:
            tag_id = result[0]
        else:
    ...
    def AddLocationTag(self,Object_id,tag_id):
        sql = "INSERT INTO TagStorage (entity_realm,entity_id,tag_id) VALUES ('object', %d, %d)" % (Object_id, tag_id)
        self.db_insert(sql)


    def DelLocationTag(self,Object_id):
        sql = "DELETE FROM TagStorage WHERE entity_id=%d and entity_realm='object'" % (Object_id)
        self.db_insert(sql)
    ...
    def GetRackId(self,location,row,rackname):
        sql = "SELECT id FROM Rack where location_name='%s' and row_name='%s' and name='%s' " % (location,row,rackname)
        result = self.db_query_one(sql)
        if result != None:
            rack_id = result[0]
        else:
    ...
    def GetObjIdRack(self,rack_id,unit):
        sql = "select object_id from RackSpace where rack_id=%d and unit_no=%d;" % (rack_id,unit)
        result = self.db_query_one(sql)
        if result != None:
            object_id = result[0]
        else:
    ...
    def AddRackSpace(self,rack_id,unit,object_id):
        '''Add new object to rackspace'''
        sql = "INSERT INTO  RackSpace (rack_id,unit_no,atom,state,object_id) VALUES (%d,%d,'front','T',%d), (%d,%d,'interior','T',%d), (%d,%d,'rear','T',%d);" % (rack_id,unit,object_id,rack_id,unit,object_id,rack_id,unit,object_id)
        self.db_insert(sql)
